...
pub fn max_area_recurse(height: &[i32], max_area: i32) -> i32 {
        match (height.first(), height.last()) {
                (Some(l), Some(r)) => {
                        if l < r {
                                max_area_recurse(&height[1..], max(l * (height.len() as i32 - 1), max_area))
                        } else {
                                max_area_recurse(&height[..height.len()-1], max(r * (height.len() as i32 - 1), max_area))
                        }
                }
                _ => max_area
        }
